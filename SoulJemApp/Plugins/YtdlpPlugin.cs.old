...
namespace SoulJemApp.Plugins
{
    public class YtdlpPlugin
    {
        ...
        public async Task<string> DownloadOrSearchAsync(string query, string targetFolder, Action<int>? onProgress = null, bool onlyAudio = false)
        {
            ...
            string format = onlyAudio ? "-f \"bestaudio\" --extract-audio --audio-format mp3 --audio-quality 0" : "-f \"bestvideo[ext=mp4][vcodec^=avc1][height<=720]+bestaudio[ext=m4a]/best[ext=mp4]/best\" --merge-output-format mp4";
        ...
        public async Task<string> DownloadAudioCustomAsync(string url, string targetFolder, string fileName, string format, string bitrate, string hz, Action<int>? onProgress = null)
        {
            ...
            string args = $"--newline --color never --no-playlist -f \"bestaudio/best\" --extract-audio --audio-format {format} --audio-quality {qualityVal} --postprocessor-args \"-ar {hz}\" -o \"{outputTemplate}\" \"{url}\"";
